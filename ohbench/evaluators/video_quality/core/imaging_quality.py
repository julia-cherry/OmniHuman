...
from .utils import try_load_video, load_dimension_info, log_skip_bad_video
...
def technical_quality(model, video_list, device, **kwargs):
    ...
    for video_path in tqdm(video_list, disable=get_rank() > 0):
        images = try_load_video(video_path)
        if images is None:
            skipped += 1
            continue
        try:
            ...
            skipped += 1
            continue
    if skipped and get_rank() <= 0:
        print(f"[imaging_quality] skipped {skipped} samples in this batch (paths printed above)", flush=True)
    if not video_results:
        raise RuntimeError(
            "No samples were processed successfully; fix paths shown in log messages above and retry."
        )
