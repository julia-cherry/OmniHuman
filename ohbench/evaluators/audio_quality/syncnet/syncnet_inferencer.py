...
class SyncnetInferencer:
    ...
    def infer(self, video_path, sr=16000, fps=25.0, batch_size=20, vshift=15, temp_audio_path="audio.wav"):
        ...
        for scene in scenes:
            ...
            distss.append(dists)
        shutil.rmtree(work_dir, ignore_errors=True)
        # Per-scene dists may differ in length (frame count); np.array(distss) can be inhomogeneous
        try:
            dists_out = np.array(distss).mean(0).mean(0).tolist()
        except ValueError:
            # If shapes differ: mean over frames per scene, then mean over scenes
            dists_out = np.array([d.mean(0) for d in distss]).mean(0).tolist()
        return sum(offsets) / len(offsets), sum(confs) / len(confs), dists_out
