...
def log_skip_bad_video(video_path, exc):
    """Print full path for failed samples (shared by imaging_quality / dynamic_degree)."""
    if get_rank() <= 0:
        print(
            "[video_quality] bad sample path:\n"
            f"{video_path}\n"
            f"({type(exc).__name__}: {exc})",
            flush=True,
        )
    logger.warning("skipped sample %s (%s: %s)", video_path, type(exc).__name__, exc)


def try_load_video(video_path, **kwargs):
    """Like load_video; on error log the path and return None."""
    try:
        return load_video(video_path, **kwargs)
    except Exception as e:
        log_skip_bad_video(video_path, e)
        return None
