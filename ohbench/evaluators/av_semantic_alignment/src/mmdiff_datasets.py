...
try:
    ...
    warnings.warn('Import mpi4py failed. Download the packages via `conda install -c conda-forge mpich mpi4py`')
import numpy as np
import torch as th
import os
import pickle
...
class MultimodalDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        video_after_process = ((video_after_process + 1) * 127.5).clamp(0, 255).to(th.uint8)
        return {"video": video_after_process, "audio": audio, "index": idx}
